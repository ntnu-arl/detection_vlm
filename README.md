...
The Humble branch is documented here. For ROS Noetic, see the
[noetic branch](https://github.com/ntnu-arl/detection_vlm/tree/noetic).
...
The launch files expose a `python_env` argument. Point it to your virtualenv
...
- `compressed_image`: subscribe to `sensor_msgs/msg/CompressedImage` when
  `true`, otherwise `sensor_msgs/msg/Image`.
- `target_frame`, `camera_frame`, `body_frame`: frame names used for TF lookup.
- `lidar_to_body_transform`: static LiDAR-to-body transform kept in the YAML
  config for this ROS 2 branch.
...
- TF from `body_frame` to `target_frame`, plus the configured
  `lidar_to_body_transform`.
...
- `publish_debug_clusters`: publish the colored point clusters used for
  association.
...
- `/semantic_map/detections_image`: annotated 2D detections used to update the
  map.
- `/semantic_map/semantic_map`: transient-local `sensor_msgs/msg/PointCloud2`
  containing active semantic voxels colored by label.
- `/semantic_map/semantic_objects`: transient-local
  `visualization_msgs/msg/MarkerArray` containing clustered semantic objects.
...
- `semantic_cluster_eps`, `semantic_cluster_min_points`,
  `semantic_object_min_voxels`, `semantic_object_min_confidence`: object
...
The reasoning node sends the input image and a binary question to an OpenAI VLM.
It publishes a visualization image with the answer probability and explanation.
